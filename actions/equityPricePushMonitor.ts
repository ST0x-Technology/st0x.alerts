...
function getActiveSession(nowUtcMs: number): ActiveSession | null {
	...
	const tradingDayStatus =
		earlyCloseHour !== null ? `Early close – regular market ends ${earlyCloseHour}:00 ET` : 'Normal trading day';
...
async function fetchRecentTransactions(context: Context, sinceUtcMs: number): Promise<TxRecord[]> {
	...
	const networkId = await context.secrets.get('TENDERLY_NETWORK_ID');

	if (!accessKey || !accountSlug || !projectSlug || !networkId) {
		throw new Error(
			'Missing Tenderly secrets — ensure TENDERLY_ACCOUNT_SLUG, TENDERLY_PROJECT_SLUG, and TENDERLY_NETWORK_ID are set'
		);
	}
...
async function hasValidPush(context: Context, session: ActiveSession): Promise<boolean> {
	// Optional: restrict to specific oracle contract address
	const rawOracle = await context.secrets.get('ORACLE_CONTRACT_ADDRESS').catch(() => null);
	...
	for (const tx of txs) {
		...
		if (tx.timestampMs < session.sessionStartUtcMs || tx.timestampMs >= session.sessionEndUtcMs) continue;
...
async function sendTelegram(context: Context, text: string): Promise<void> {
	...
	if (!token || !chatId) {
		console.warn('Telegram secrets not configured (TELEGRAM_BOT_TOKEN / TELEGRAM_CHAT_ID)');
		return;
	}
...
function buildAlertMessage(session: ActiveSession, nowUtcMs: number): string {
	...
	const sessionEnd = fmtEtTime(session.effectiveEndHour, session.effectiveEndMin);
...
export const handleEquityPricePushMonitor: ActionFn = async (context: Context, _event: Event) => {
	...
	console.log(`[equity-monitor] Session: ${session.def.name} (${session.dateStr}), past grace: ${session.pastGrace}`);

	if (!session.pastGrace) {
		console.log(`[equity-monitor] Grace period not elapsed (deadline ${etDisplayTime(session.graceDeadlineUtcMs)}).`);
		return;
	}

	// Dedup: alert at most once per session per day
	const storageKey = `equity_push_monitor:${session.dateStr}:${session.def.name}`;
	const stored = (await context.storage.getJson(storageKey).catch(() => null)) as Record<string, unknown> | null;

	if (stored?.alerted === true) {
		console.log(`[equity-monitor] Already alerted for ${session.def.name} on ${session.dateStr}.`);
		return;
	}
	...
	if (pushFound) {
		console.log(`[equity-monitor] Valid push found for ${session.def.name} on ${session.dateStr}.`);
		await context.storage.putJson(storageKey, { alerted: false, pushFoundAtUtcMs: nowUtcMs });
		return;
	}
	...
	await sendTelegram(context, msg);
	await context.storage.putJson(storageKey, { alerted: true, alertedAtUtcMs: nowUtcMs });
};
